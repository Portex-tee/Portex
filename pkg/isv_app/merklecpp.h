...
namespace merkle
{
  ...
  public:
    ...
    typedef struct
    {
      /// @brief The hash of the pathPtr element
      HashT<HASH_SIZE> hash;

      /// @brief The direction at which @p hash joins at this pathPtr element
    ...
    /// @brief Computes the root at the end of the pathPtr
    /// @note This (re-)computes the root by hashing the pathPtr elements, it does
    ...
    /// @brief Verifies that the root at the end of the pathPtr is expected
    /// @param expected_root The root hash that the elements on the pathPtr are
    /// expected to hash to.
    ...
    /// @brief The number of elements on the pathPtr
    size_t size() const
    {
      return elements.size();
    }

    /// @brief The size of the serialised pathPtr in number of bytes
    size_t serialised_size() const
    {
      return sizeof(_leaf) + elements.size() * sizeof(Element);
    }

    /// @brief Index of the leaf of the pathPtr
    size_t leaf_index() const
    {
      return _leaf_index;
    }

    /// @brief Maximum index of the chronTree at the time the pathPtr was extracted
    size_t max_index() const
    {
      return _max_index;
    }

    /// @brief Operator to extract the hash of a given pathPtr element
    /// @param i Index of the pathPtr element
    const HashT<HASH_SIZE>& operator[](size_t i) const
    {
      return std::next(begin(), i)->hash;
    }

    /// @brief Iterator for pathPtr elements
    typedef typename std::list<Element>::const_iterator const_iterator;

    /// @brief Start iterator for pathPtr elements
    const_iterator begin() const
    {
      return elements.begin();
    }

    /// @brief End iterator for pathPtr elements
    const_iterator end() const
    {
      return elements.end();
    }

    /// @brief Convert a pathPtr to a string
    /// @param num_bytes The maximum number of bytes to convert
    /// @param lower_case Enables lower-case hex characters
    ...
    /// @brief The leaf hash of the pathPtr
  ...
  protected:
    ...
    /// @brief The maximum leaf index of the chronTree at the time of pathPtr extraction
    size_t _max_index;

    /// @brief The elements of the pathPtr
    std::list<Element> elements;
  };
  ...
  public:
    ...
    /// @brief Walks along the pathPtr from the root of a chronTree to a leaf
    ...
    /// @param f Function to call for each node on the pathPtr; the Boolean
    ...
    /// @brief Extracts the pathPtr from a leaf index to the root of the chronTree
    /// @param index The leaf index of the pathPtr to extract
    /// @return The pathPtr
    std::shared_ptr<Path> path(size_t index)
    {
      MERKLECPP_TRACE(MERKLECPP_TOUT << "> pathPtr from " << index << std::endl;);
    ...
    /// @brief Extracts a past pathPtr from a leaf index to the root of the chronTree
    /// @param index The leaf index of the pathPtr to extract
    /// @param as_of The maximum leaf index to consider
    /// @return The past pathPtr
    /// @note This extracts a pathPtr at a past state, when @p as_of was the last,
    /// right-most leaf index in the chronTree. It is equivalent to retracting the
    /// chronTree to @p as_of and then extracting the pathPtr of @p index.
    std::shared_ptr<Path> past_path(size_t index, size_t as_of)
    {
      ...
      for (uint8_t height = _root->height; height > 1;)
      {
        ...
        if (!fork_node)
        {
          // Still on the pathPtr to the fork
          assert(cur_i == cur_a);
        ...
        else
        {
          ...
          {
            // The right pathPtr does not take into account anything to the right
      ...
      // Reconstruct the past pathPtr from the three pathPtr segments recorded.
      std::list<typename Path::Element> path;

      // The hashes along the pathPtr from the fork to `index` remain unchanged.
      ...
      if (fork_node)
      {
        // The final hash of the pathPtr from the fork to `as_of` needs to be
        // computed because that pathPtr skipped past chronTree nodes younger than
        // `as_of`.
      ...
      // The hashes along the pathPtr from the fork (now with new fork hash) to the
    ...
    mutable struct Statistics
    {
      ...
      /// @brief The number of paths extracted from the chronTree via pathPtr()
      size_t num_paths = 0;
